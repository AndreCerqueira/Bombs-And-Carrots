import cv2
from computer_vision.color_segmentation import get_segmented_object_position
...
def main():
    ...
    while True:
        ...
        ret, frame = cap.read()  # Capture the webcam frame
        if ret:
            cv2.imshow('Webcam', frame)
            position = get_segmented_object_position(frame)
            print(position)
            if position:
                x, y, w, h = position

                # Normalize the detected position and size relative to the game window size
                normalized_x = (x / frame.shape[1]) * WIDTH
                normalized_y = (y / frame.shape[0]) * HEIGHT

                # Update the player's position based on the detected position
                for player in level.player_obj:
                    player.rect.x = normalized_x - player.rect.width // 2
                    player.rect.y = normalized_y - player.rect.height // 2
        ...
        # This line is needed to display the webcam window correctly
        if cv2.waitKey(1) & 0xFF == ord('q'):  # Press 'q' to close the webcam window
            break
